...
<page backtop="20mm" backleft="10mm" backright="10mm" backbottom="0mm">
    ...
    <div style="box-shadow: 0px 0px 2px 5px; border-radius: 31px; width: 100%; text-align: center; margin-top: 50px; color: white; background-color:#697889;  ">
        <h1>Journal Mensuel : Diverse Contributions <?= $monthname ?> </h1>
    </div>
    <table style="width: 400px; margin-top:60px; margin-left: 0px;">
        ...
        foreach ($printlist as $mem) {
            ?>
            <tr class="animate">
                <td style="width:20%; text-align: center;"class="bord"><?= $i; ?></td>
                <td style="width:60%;"class="bord"> <?= $mem->getfullname() ?> </td>
                <td style="width:40%;text-align: center;"class="bord"> <?= $mem->getdate() ?> </td>
                <td style="width:50%;text-align: center;"class="bord"> <?= number_format($mem->getamount()) ?> </td>
            </tr>
            <?php
            $tot += $mem->getamount();
            $i++;
        ...
        ?>
            <tr style="border-radius: 2px 2px 0px 0px; background:#697889; text-align:center; color:white; ">
                <td colspan="3" style="width:10%; text-align: center;"class="bord">Totale</td>
                <td  style="width:10%; text-align: center;"class="bord"><?= number_format($tot) ?></td>
            </tr>
    </table>
...
try {
    ...
    $pdf->output('Rapports mensuel C-diverse.pdf', 'I');
