...
<page backtop="20mm" backleft="10mm" backright="10mm" backbottom="0mm">
    ...
    <div style="box-shadow: 0px 0px 2px 5px; border-radius: 31px; width: 100%; text-align: center; margin-top: 50px; color: white; background-color:#697889;  ">
        <h1>Journal Générale : Droits D'Adhésion</h1>
    </div>
    <table style="width: 400px; margin-top:60px; margin-left: -10px;">
        ...
        foreach ($printlist as $mem) {
            ...
            <tr class="animate">
                ...
                <td style="width:65%;"class="bord"> <?= $mem->getfullname() ?> </td>
                <td style="width:45%;text-align: center;"class="bord"> <?= $mem->getdate() ?> </td>
                <td style="width:55%;text-align: center;"class="bord"> <?= number_format($mem->getamount()) ?> </td>
            </tr>
            <?php
            $amt += $mem->getamount();
            $i++;
...
try {
    ...
    $pdf->output("Journal Droits d'Adhésion.pdf", 'I');
