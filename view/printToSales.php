...
<page backtop="20mm" backleft="10mm" backright="10mm" backbottom="0mm">
    ...
    <div style="box-shadow: 0px 0px 2px 5px; border-radius: 31px; width: 100%; text-align: center; margin-top: 50px; color: white; background-color:#697889;  ">
        <h1>Journal Mensuel : Ventes <?= $par['tontine'] . ' ' . $monthname ?></h1>
    </div>
    <table style="width: 400px; margin-top:60px; margin-left: -10px;">
        ...
        foreach ($printlist as $mem) {
            ...
            <tr class="animate">
                ...
                <td style="width:35%;text-align: center;"class="bord"> <?= number_format($mem->getamount()) . ' FCFA' ?> </td>
                <td style="width:35%;text-align: center;"class="bord"> <?= number_format($mem->getsellingprice()) . ' FCFA' ?> </td>
            </tr>
            <?php
            $lot += $mem->getamount();
        ...
        <tr style="background:#697889; color: white;">
            <td colspan="4" style="text-align: center; color: white;"> Totale</td>
